...
| GET /items?from=...&size=... | получение списка своих вещей. Номер первой вещи списка - from, длина списка - size. Параметры from и size не обязательные |
| GET /items/search?text=...&from=...&size=... | поиск вещей по названию или описанию. Номер первой вещи списка - from, длина списка - size. Параметры from и size не обязательные |
| POST /items/{itemId}/comment | добавление комментария к вещи, которую использовал |
| POST /bookings | бронирование вещи |
| PATCH /bookings/{bookingId}?approved=... | изменение статуса брони владельцем |
| GET /bookings/{bookingId} | получение информации о брони владельцем |
| GET /bookings?state=...&from=...&size=... | получении информации о забронированных пользователем вещах с определенным статусом бронирования. Номер первой вещи списка - from, длина списка - size. Параметры state, from и size не обязательные |
| GET /bookings/owner?state=...&from=...&size=.. | получении информации о бронировании своих вещей другими пользователями с определенным статусом бронирования. Номер первой вещи списка - from, длина списка - size. Параметры state, from и size не обязательные |
