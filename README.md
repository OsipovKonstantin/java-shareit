...
| GET /items?from=...&size=... | получение списка своих вещей. Номер первой вещи списка - from, длина списка - size |
| GET /items/search?text=...&from=...&size=... | поиск вещей по названию или описанию. Номер первой вещи списка - from, длина списка - size |
| POST /items/{itemId}/comment | добавление комментария к вещи, которую использовал |
| POST /bookings | бронирование вещи |
| PATCH /bookings/{bookingId} | изменение статуса брони владельцем |
| GET /bookings/{bookingId} | получение информации о брони владельцем |
| GET /bookings | изменение статуса брони владельцем |
| GET /bookings/owner | изменение статуса брони владельцем |
